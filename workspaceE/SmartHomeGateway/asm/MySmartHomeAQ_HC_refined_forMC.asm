...
//functions added in refinement
	...
	derived overallAQInsideCO2: Powerset(SysAMdA) -> Co2Domain
...
function airLevel ($v in AirDomain) = 
	...
	//function floorname($a in SysAMdA) = if $a = aqs_gf then "_GF" else  "_FF" endif

    //Added in refinement for debugging/logging
	//function floorname($a in SysHMdA) = if $a = hs_gf then "_GF" else  "_FF" endif
